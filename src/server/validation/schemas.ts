...
export const paginationSchema = z.object({ page: z.coerce.number().int().min(1).default(1), pageSize: z.coerce.number().int().min(1).max(100).default(50), search: z.string().trim().max(100).default(""), status: z.string().max(50).optional(), sort: z.string().max(50).optional() });
...
export const packageListSchema = paginationSchema.extend({
  dateFrom:z.iso.date().optional(),dateTo:z.iso.date().optional(),status:z.enum(["WAITING_CLOSING","READY_TO_SHIP","IN_TRANSIT","ARRIVED_MERAUKE","READY_FOR_PICKUP","COMPLETED","HOLD","DAMAGED","MISSING"]).optional(),
  sort:z.enum(["received_desc","received_asc","fee_desc","fee_asc"]).default("received_desc")
});
