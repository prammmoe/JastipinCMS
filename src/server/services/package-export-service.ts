...
export async function exportPackagesCsv(request: NextRequest) {
  let query = createAdminClient()
    .from("packages")
    .select("package_code,tracking_number,status,received_date,received_time,actual_weight_kg,chargeable_weight_kg,shipping_fee_idr,customers(code,name)")
  ...
  const lines = [
    "PACKAGE CODE,TRACKING NUMBER,CUSTOMER CODE,CUSTOMER,STATUS,RECEIVED DATE,RECEIVED TIME,ACTUAL WEIGHT,CHARGEABLE WEIGHT,SHIPPING FEE",
    ...(result.data ?? []).map((row) => {
      const customer = row.customers as unknown as { code: string; name: string } | null;
      return [
        row.package_code,
        row.tracking_number,
        customer?.code,
