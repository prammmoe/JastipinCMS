...
const statusByCode: Record<string, number> = {
  ...
  CUSTOMER_NAME_EXISTS: 409,
  CLOSING_NOT_FOUND: 404,
  SHIPMENT_NOT_FOUND: 404,
  PACKAGE_NOT_ELIGIBLE_FOR_CLOSING: 409,
  CLOSING_ALREADY_FINALIZED: 409,
  CLOSING_EMPTY: 400,
  CLOSING_DUPLICATE_PACKAGE: 409,
  PACKAGE_ALREADY_IN_CLOSING: 409,
  CLOSING_NOT_CHECKABLE: 409,
  CROSSCHECK_EMPTY: 400,
  PACKAGE_NOT_CHECKABLE: 409,
  PACKAGE_NOT_IN_SHIPPING: 409,
  INVALID_CONDITION: 400,
  SHIPMENT_NOT_DEPARTABLE: 409,
  PACKAGE_NOT_IN_SHIPMENT: 409,
  ARRIVAL_ALREADY_SCANNED: 409,
  PAYMENT_EXCEEDS_BALANCE: 409,
  PICKUP_PACKAGE_NOT_READY: 409,
...
export class AppError extends Error {
  constructor(
    ...
    public readonly status = statusByCode[code] ?? 400,
  ) {
    super(message);
  }
}
...
export function mapDatabaseError(
  ...
  const known = [
    "PACKAGE_NOT_ELIGIBLE_FOR_CLOSING",
    "CLOSING_NOT_DRAFT",
    "CLOSING_HAS_PAYMENTS",
    "SHIPMENT_NOT_DEPARTABLE",
    "SHIPMENT_NOT_RECONCILABLE",
    "MISSING_CONFIRMATION_REQUIRED",
    "PAYMENT_EXCEEDS_BALANCE",
    "PICKUP_PACKAGE_NOT_READY",
    "PICKUP_EMPTY",
    "PACKAGE_ATTACHMENT_LIMIT",
    "CLOSING_EMPTY",
    "CLOSING_DUPLICATE_PACKAGE",
    "PACKAGE_ALREADY_IN_CLOSING",
    "CLOSING_NOT_CHECKABLE",
    "PACKAGE_NOT_CHECKABLE",
    "PACKAGE_NOT_IN_SHIPPING",
    "INVALID_CONDITION",
  ];
  const code =
    known.find((candidate) => raw.includes(candidate)) ??
